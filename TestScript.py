...
def run (url = 'http://127.0.0.1:5000'):
    ...
    print("test case 2")
    test_job_config = {'run_now': True, 'retries': 4, 'priority': 6, 'memory': 4096}
    new_future2 = testInterface.submit('test_case_2', job_config = test_job_config)
    print('future data: \n\t' + str(new_future2.future_info()))
    id_to_grab = str((new_future2.future_info()).get('job_id'))
    print("grabbed data: \n\t" + str(testInterface._grab(id_to_grab)))
